...
def parse_args(dict_args: Union[Dict[str, Any], None]) -> argparse.Namespace:
    ...
    parser.add_argument(
        "--type", type=int, required=True, choices=[1, 2], help="Choose script type: 1 or 2"
    )
    parser.add_argument(
        "--model_type",
        type=str,
    ...
    parser.add_argument(
        "--out_name", type=str, default="final", help="Path to directory with results as wav file"
    )
...
def type_1_main(
    args: argparse.Namespace, device: str, model: nn.Module, config: Any, chunk_size: int
) -> None:
    """
