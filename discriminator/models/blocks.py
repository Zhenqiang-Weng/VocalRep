...
class ResidualBlock(nn.Module):
    ...
    def __init__(
        self, channels, kernel_size, dilation, n=2, causal=False, norm="freq", activation="relu"
    ):
...
class GatedConvBlock(nn.Module):
    ...
    def __init__(
        self, channels, kernel_size, dilation, causal=False, norm="freq", activation="relu"
    ):
        super(ResidualBlock, self).__init__()
        ...
        if self.generate and self.causal:
            if self.tensor_q is None:
                ...
                assert residual.shape[1] == 1, (
                    f"Expected residual.shape[1] == 1 during generation, but got residual.shape[1]={residual.shape[1]}"
                )
...
class FFN(nn.Module):
    def __init__(self, in_channels, out_channels, kernel_size, p_dropout=0.0, activation=None):
        super().__init__()
    ...
    def __init__(
        ...
        for i in range(self.n_layers):
            ...
            self.ffn_layers.append(
                FFN(hidden_channels, hidden_channels, kernel_size, p_dropout=p_dropout)
            )
...
class ScaledDotProductAttention(nn.Module):
    ...
    def forward(self, q, k, v, key_mask=None, query_mask=None, mapping_mask=None, attn_prior=None):
        ...
        if key_mask is not None:
            attn = attn.masked_fill(key_mask == 0.0, -np.inf)
        if attn_prior is not None:
...
def word_level_pooling(src_seq, src_len, wb, src_w_len, reduce="sum"):
    ...
    for s, sl, w, wl in zip(src_seq, src_len, wb, src_w_len):
        ...
        if reduce == "sum":
            ...
            m = torch.div(
                torch.sum(m, dim=0), torch.tensor(split_size, device=device).unsqueeze(-1)
            )  # [src_w_len, hidden]
        else:
            raise ValueError()
        batch.append(m)
