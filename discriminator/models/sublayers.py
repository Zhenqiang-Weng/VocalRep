...
from discriminator.utils.functional import (
    create_positions,
    positional_encoding,
...
class VanillaUpsampler(nn.Module):
    ...
    def forward(self, x, durations):
        ...
        out = torch.matmul(weights.transpose(1, 2), x)  # [B, T, DIM]
...
class VanillaDownsampler(nn.Module):
    ...
    def forward(self, x, durations):
        ...
        out = torch.matmul(weights, x.transpose(1, 2))  # [B, N, Mel]
        positions = create_positions(durations, mode=self.mode).to(x.device)  # [B, T]
...
class GaussianUpsampler(nn.Module):
    ...
    def forward(self, x, durations, ranges):
        ...
        w_2 = (
            torch.sum(
                torch.exp(-(ranges.unsqueeze(-1) ** -2) * ((t - c) ** 2)), dim=1, keepdim=True
            )
            + 1e-20
        )  # [B, N, 1]
...
class ScaledDotAttention(nn.Module):
    def __init__(
        self, in_channels, hidden_channels, out_channels, noise=0, normalize=False, dropout=False
    ):
    ...
    def forward(self, q, k, v, mask=None):
        ...
        if self.normalize:
            q = self.qnorm(q)
            k = self.knorm(k)
        result, weights = scaled_dot_attention(
            self.fc_query(q),
            self.fc_keys(k),
            self.fc_values(v),
            mask,
