...
class TextEncoder(nn.Module):
    ...
    def __init__(self, hp):
        ...
        self.res_blocks = nn.Sequential(
            *[
                ResidualBlock(
                    hp.channels, hp.kernel_size, d, n=2, norm=hp.normalize, activation=hp.activation
                )
...
class SpecDecoder(nn.Module):
    ...
    def __init__(self, hp):
        ...
        self.post_net2 = nn.Sequential(
            ResidualBlock(
                hp.channels, self.kernel_size, 1, n=2, norm=hp.normalize, activation=hp.activation
            ),
...
class MoEPredictor(nn.Module):
    def __init__(self, channels, out_channels=1, experts=3, kernel=3, norm="freq", dropout=0.1):
        ...
        self.router = VariancePredictor(
            channels, out_channels=experts, kernel=kernel, norm=norm, dropout=dropout
        )
        self.experts = nn.ModuleList(
            [
                VariancePredictor(channels, out_channels, kernel=kernel, norm=norm, dropout=dropout)
...
class RhythmAdaptor(nn.Module):
    ...
    def __init__(self, hp):
        ...
        if hp.enable_speed_moe:
            self.duration_predictor = MoEPredictor(
                hp.channels,
                experts=len(hp.speed_boundaries) - 1,
                kernel=hp.enc_kernel_size,
                norm=hp.normalization,
                dropout=hp.dropout,
            )
        else:
            self.duration_predictor = VariancePredictor(
                hp.channels, kernel=hp.enc_kernel_size, norm=hp.normalization, dropout=hp.dropout
            )
    ...
    def get_durations(self, durations, lengths, control):
        ...
        durations = torch.clamp(
            torch.round(durations),
            min=self.hparams.min_frames_per_phoneme,
            max=self.hparams.max_frames_per_phoneme,
        ).long()
    ...
    def add_position_enc(self, src_seq, position_enc=None, coef=None):
        ...
        if not self.training and src_seq.shape[1] > self.max_seq_len:
            pos_enc = (
                get_sinusoid_encoding_table(src_seq.shape[1], self.d_model)[: src_seq.shape[1], :]
                .unsqueeze(0)
            ...
            pos_enc = position_enc[:, :max_len, :].expand(batch_size, -1, -1)
            if coef is not None:
                pos_enc = coef.unsqueeze(-1) * pos_enc
            enc_output = src_seq + pos_enc
        return enc_output
    ...
    def get_mapping_mask(self, q, kv, dur_w, wb, src_w_len):
        ...
        for b, (w, p, l) in enumerate(zip(dur_w, wb, src_w_len)):
            w, p = (
                [0] + [d.item() for d in torch.cumsum(w[:l], dim=0)],
                [0] + [d.item() for d in torch.cumsum(p[:l], dim=0)],
            )
            # assert len(w) == len(p)
            for i in range(1, len(w)):
                mask[b, w[i - 1] : w[i], p[i - 1] : p[i]] = torch.zeros(
                    w[i] - w[i - 1], p[i] - p[i - 1], device=device
                )
        return mask == 0.0
    ...
    def get_rel_coef(self, dur, dur_len, mask):
        ...
        for d, dl in zip(dur, dur_len):
            idx_b, d = [], d[:dl].long()
            m = torch.repeat_interleave(d, torch.tensor(list(d), device=device), dim=0)  # [tgt_len]
            L.append(m)
            for d_i in d:
                idx_b += list(range(d_i))
            idx.append(torch.tensor(idx_b).to(device))
            # assert L[-1].shape == idx[-1].shape
        return torch.div(pad(idx).to(device), pad(L).masked_fill(mask == 0.0, 1.0).to(device))
    ...
    def forward(
        ...
        enc_w_out = self.word_encoder(
            src_w_seq.transpose(1, 2), src_w_masks.unsqueeze(1)
        ).transpose(1, 2)

        duration_inputs = (
            encodings.detach() if self.hparams.separate_duration_grad else encodings
        )  # [B, N, DIM]
        if self.hparams.enable_speed_moe:
            duration_outputs, route_weights = self.duration_predictor(
                duration_inputs
            )  # [B, N, 1], [B, N, N_exp]
        else:
        ...
        log_duration_w_prediction = torch.log(
            word_level_pooling(
                duration_outputs.exp().unsqueeze(-1),
                input_lengths,
                word_boundaries,
                src_w_lens,
                reduce="sum",
        ...
        q = self.add_position_enc(
            x,
            position_enc=self.q_position_enc,
            coef=self.get_rel_coef(duration_w_rounded, src_w_lens, mel_masks),
        )
        k = self.add_position_enc(
            encodings,
            position_enc=self.kv_position_enc,
            coef=self.get_rel_coef(word_boundaries, input_lengths, src_masks),
        )
