...
class CC(nn.Module):
    ...
    def forward(self, inputs):
        ...
        (encodings, output_lengths, alignment_outputs, feature_outputs) = self.adaptor(
            encodings,
            duration,
            features,
            voice,
        ...
        if self.hparams.enable_ssim_loss:
            ssim_loss = (
                ...
                if use_dtw
                    ...
                    [
                        masked_ssim(spec_outputs, specs, output_lengths, window_size=w)
                        for w in self.hparams.ssim_window_size
                    ]
                )
            )
