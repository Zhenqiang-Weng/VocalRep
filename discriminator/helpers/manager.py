...
def check_gpus():
    ...
    if not torch.cuda.is_available():
        print(
            "This script could only be used to manage NVIDIA GPUs, but not GPU found in your device!"
        )
        return False
...
if check_gpus():
    ...
    def parse(line, qargs):
        ...
        Parse a CSV line returned by nvidia-smi
        """
        numberic_args = [
            "memory.free",
            "memory.total",
        ...
        )  # Check GPU power management support, which may be absent on laptops
        to_numberic = lambda v: float(
            v.upper().strip().replace("MIB", "").replace("W", "")
        )  # Strip units from numeric strings
        process = lambda k, v: (
    ...
    class GPUManager:
        ...
        ones pref.
        List and sort available GPUs to select the least busy device.
        Track assignments within each GPUManager instance,
        preferring GPUs that have not already been assigned.
        """
        ...
        def _sort_by_memory(self, gpus, by_size=False):
            if by_size:
                ...
                return sorted(
                    gpus, key=lambda d: float(d["memory.free"]) / d["memory.total"], reverse=True
                )
        ...
        def auto_choice(self, mode=0):
            ...
            Select the least busy GPU and return its index
            """
