...
class CCTrainer(Trainer):
    def __init__(self, hparams, device=torch.device("cuda")):
        ...
        if (self.data_hparams.load_data_from_disk is False) or (
            self.data_hparams.compute_statistics is True
        ):
        ...
        scheduler = ReduceLROnPlateau(
            optimizer, factor=self.optim_hparams.factor, patience=self.optim_hparams.patience
        )
    ...
    def fit(self, checkpoint=None, loggers=None):
        ...
        train_loader = self.dataloader(
            self.trainset, batch_size=self.data_hparams.batch_size, collate_fn=collate_fn
        )
        valid_loader = self.dataloader(
            self.validset, batch_size=self.data_hparams.batch_size, collate_fn=collate_fn
        )
        ...
        for it, batch in enumerate(pbar, start=1):
            ...
            if self.step % self.optim_hparams.grad_acc_step == 0:
                ...
                torch.nn.utils.clip_grad_norm_(
                    self.model.parameters(), self.optim_hparams.grad_clip_thresh
                )
                self.optimizer.step()
            ...
            pbar.set_postfix({key: f"{val:.5f}" for key, val in epoch_losses.items()})

            # logging
            learning_rate = self._get_learning_rate(self.optimizer)

            self.loggers.log_train(
                "train", "step", self.step, losses, lr_dict={"lr", learning_rate}
            )
    ...
    def _validate(self, dataloader):
        ...
        for it, batch in enumerate(pbar, start=1):
            ...
            specs_fig, _ = plot_spectrogram(
                spec_outputs[index, :predicted_length, :], target=specs[index, :output_length, :]
            )

            self.loggers.log_train(
                "valid", "step", self.step, losses, image_dict={"spec": specs_fig}
            )
    ...
    def save_checkpoint(self, remove=True):
        ...
        checkpoint = os.path.join(
            self.train_hparams.out_path,
            f"{time.strftime('%Y-%m-%d')}_chkpt_step{self.step:05d}.pth",
        )
...
class CCGANTrainer(Trainer):
    def __init__(self, hparams, device=torch.device("cuda")):
        ...
        self.d_scheduler = NoamScheduler(
            self.d_optimizer,
    ...
    def _train_epoch(self, dataloader, base_step):
        ...
        for it, batch in enumerate(pbar, start=1):
            ...
            if self.step >= self.disc_hparams.n_train_start and lengths[1].min >= max(
                ...
                gan_loss = self.d_criterion(
                    d_fake, torch.tensor(1.0).expand_as(d_fake).to(d_fake.device)
                )
                losses["gan"] = gan_loss * self.disc_hparams.gan_loss_lambda
                # feature mapping loss
                if self.disc_hparams.enable_hdn_loss:
                    *__, h_real = self.d_model(
                        targets[0], lengths[1], start_frames_wins=start_frames_wins
                    )
                    h_fake = [h_in for h_out in h_fake for h_in in h_out]
                    h_real = [h_in for h_out in h_real for h_in in h_out]
                    hdn_loss = self.d_criterion(
                        torch.stack(h_fake, dim=-1), torch.satack(h_real, dim=-1)
                    )
            ...
            if self.step >= self.disc_hparams.n_train_start and lengths[1].min >= max(
                ...
                d_real_loss = self.d_criterion(
                    d_real, torch.tensor(1.0).expand_as(d_real).to(d_real.device)
                )
                d_fake_loss = self.d_criterion(
                    d_fake, torch.tensor(0.0).expand_as(d_fake).to(d_fake.device)
                )
                ...
                if self.step % self.disc_hparams.grad_acc_step == 0:
                    # clipping gradients to avoid gradient explosion
                    nn.utils.clip_grad_norm_(
                        self.d_model.parameters(), self.disc_hparams.grad_clip_thresh
                    )
            ...
            if (self.step - base_step) % self.train_hparams.target_end_step:
                os.symlink(
                    self.checkpoint,
                    os.path.join(
                        self.train_hparams.out_path, f"step_{self.train_hparams.target_end_step}"
                    ),
                )
