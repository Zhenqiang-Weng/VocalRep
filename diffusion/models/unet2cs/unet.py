    ...
    def __init__(
        ...
        if time_emb_dim is not None:
            self.time_mlp = nn.Sequential(
                nn.Linear(time_emb_dim, out_channels), nn.ReLU(inplace=True)
            )
        else:
            self.time_mlp = None
...
class ResEncoderBlock(nn.Module):
    ...
    def __init__(
        ...
        in_channels: int,
        out_channels: int,
        pool_kernel: Optional[Tuple[int, int]],
        n_blocks: int = 1,
        time_emb_dim: Optional[int] = None,
        momentum: float = 0.01,
    ):
        ...
        layers = []
        layers.append(
            ConvBlockRes(in_channels, out_channels, time_emb_dim=time_emb_dim, momentum=momentum)
        )
        for _ in range(n_blocks - 1):
            layers.append(
                ConvBlockRes(
                    out_channels, out_channels, time_emb_dim=time_emb_dim, momentum=momentum
                )
            )
...
class ResDecoderBlock(nn.Module):
    ...
    def __init__(
        ...
        convs.append(
            ConvBlockRes(
                out_channels * 2, out_channels, time_emb_dim=time_emb_dim, momentum=momentum
            )
        )
        for _ in range(n_blocks - 1):
            convs.append(
    ...
    def forward(
        self, x: torch.Tensor, skip: torch.Tensor, time_emb: Optional[torch.Tensor] = None
    ) -> torch.Tensor:
        x = self.up(x)
...
class Decoder(nn.Module):
    ...
    def forward(
        self, x: torch.Tensor, skips: List[torch.Tensor], time_emb: Optional[torch.Tensor] = None
    ) -> torch.Tensor:
...
class TimbreFilter(nn.Module):
    ...
    def __init__(
        self,
        latent_channels: List[Tuple[int, int]],
        time_emb_dim: Optional[int] = None,
        momentum: float = 0.01,
    ):
        super().__init__()
    ...
    def forward(
        self, skip_maps: List[torch.Tensor], time_emb: Optional[torch.Tensor] = None
    ) -> List[torch.Tensor]:
        out = []
...
class UNet(nn.Module):
    ...
    def __init__(
        ...
        self.intermediate = Intermediate(
            bottom_in, bottom_out, inter_layers, n_blocks, time_emb_dim=time_emb_dim
        )
