...
def extract_dominant_speaker_embedding_with_clusters(
    ...
    fine_grain_window: float = 0.5,  # Fine-grained window, defaulting to 0.5 seconds
) -> Optional[ClusteringResult]:

    model.eval()

    # Step 0: Convert stereo to mono
    if len(audio.shape) == 2:
        if audio.shape[0] == 2:
    ...
    # Step 2: Detect energy in short windows to identify active audio regions
    fine_window_samples = int(fine_grain_window * target_sr)
    total_samples = len(audio)

    # 2.1 Mark the energy of each short window
    valid_mask = []
    ...
    # 2.2 Merge contiguous active regions
    active_segments = []
    i = 0
    ...
    # Step 3: Repeat short active regions to 2 s and split long regions into 2 s segments
    ...
    for start, end in active_segments:
        ...
        if seg_len < target_samples:
            ...
            # Split long regions into 2 s segments
            for seg_start in range(0, seg_len, target_samples):
                ...
                # Repeat the final segment if it is shorter than 2 s
    ...
    # Step 4: Extract features and embeddings in batches
    features_list = []
    ...
    # Estimate the number of spectral clusters
    estimated_k, evals, gaps = estimate_k_by_eigengap(
        affinity_matrix, k_max=min(max_clusters, n_segments - 1)
    )
    ...
    # Compute the mean embedding of the largest cluster
    ...
    # Compute the maximum distance between cluster centers
    cluster_centers = []
