...
# ============== Fixed parameters ==============
FEAT_DIM = 80  # Fbank feature dimension
EMBEDDING_SIZE = 192  # Output embedding dimension
SAMPLE_RATE = 16000  # Target sample rate
SEGMENT_DURATION = 2.0  # Two-second segment

# Fbank parameters matching torchaudio.compliance.kaldi.fbank
FRAME_LENGTH_MS = 25.0  # Frame length 25ms
FRAME_SHIFT_MS = 10.0  # Frame shift 10ms

# Compute the fixed time_frames
# 2 seconds = 2000 ms, Frame shift 10ms -> (2000 - 25) / 10 + 1 = 198.5 -> 198 frames
# Alternatively, compute using samples: 2s * 16000 = 32000 samples
# frame_length = 25ms * 16 = 400 samples
# frame_shift = 10ms * 16 = 160 samples
# num_frames = (32000 - 400) / 160 + 1 = 198.75 -> 198 frames
...
class CAMPPlusExportable(nn.Module):
    """
    Exportable CAMP++ model wrapper
    Fixed input shape: (batch, TIME_FRAMES, FEAT_DIM) = (batch, 198, 80)
    Output: (batch, EMBEDDING_SIZE) = (batch, 192)
    """
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        """
        Args:
            x: (batch, 198, 80) - Fbank features of two-second audio
        Returns:
            embedding: (batch, 192)
        """
        return self.model(x)
...
def export_campplus_to_onnx(
    ...
    Fixed input: (batch, 198, 80) - Fbank features of two-second 16 kHz audio
    """
    ...
    # Create dummy input with a fixed shape
...
def onnx_to_tensorrt(
    onnx_path: str, output_path: str, fp16: bool = True, workspace_gb: int = 4
) -> Optional[str]:
    ...
    engine_size = (
        engine_bytes.nbytes if hasattr(engine_bytes, "nbytes") else len(bytes(engine_bytes))
    )
...
class SpeakerExtractorTRT:
    """Complete speaker feature extractor using TensorRT"""
    ...
    def extract_fbank(self, audio: torch.Tensor) -> torch.Tensor:
        ...
        audio: (samples,) or (batch, samples) - Must be 16 kHz
        Returns: (batch, 198, 80)
        """
        if audio.dim() == 1:
    ...
    def extract_embedding(self, audio: np.ndarray, source_sr: int = 44100) -> torch.Tensor:
        ...
        if source_sr != self.sample_rate:
            audio = scipy.signal.resample_poly(audio, self.sample_rate, source_sr).astype(
                np.float32
            )
    ...
    def extract_dominant_embedding(
        ...
        """Extract the dominant speaker embedding from long audio using clustering"""
        import scipy.signal
...
def export_campplus(model_dir: str, output_dir: str, batch_size: int = 1, fp16: bool = True):
    """Export CAMP++ to ONNX and TensorRT"""
