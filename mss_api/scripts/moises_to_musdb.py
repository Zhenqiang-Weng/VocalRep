...
def convert_dataset(
    src_root: str, dest_root: str, stems: List[str], max_folders: int = 240, num_workers: int = 4
) -> None:
    """
    Converts MoisesDB dataset to MUSDB18 format for a specified number of folders.
    Parameters:
...
def parse_args(dict_args: Union[Dict, None]) -> argparse.Namespace:
    ...
    parser.add_argument(
        "--src_dir", type=str, required=True, help="Source directory with MoisesDB tracks"
    )
    parser.add_argument(
        "--dest_dir", type=str, required=True, help="Directory to save tracks in MUSDB18"
    )
...
def main(args: Optional[argparse.Namespace] = None) -> None:
    ...
    print(f"num_workers: {num_workers}, categories: {stems + ['other']}")
