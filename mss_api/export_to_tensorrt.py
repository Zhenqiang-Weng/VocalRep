...
from models.bs_roformer.bs_roformer_exportable import (
    BDCSGBSRoformerExportable,
    convert_full_to_exportable,
)
...
def export_onnx(model_path, config_path, output_path, batch_size=1):
    ...
    # Read the correct time_frames from config
    chunk_size = config.audio.chunk_size
    hop_length = model_cfg.stft_hop_length
    n_fft = model_cfg.stft_n_fft

    # Formula when center=True：time_frames = floor(samples / hop_length) + 1
...
def onnx_to_tensorrt(onnx_path, output_path, fp16=False, bf16=False, workspace_gb=16):
    ...
    if use_reduced_precision and builder.platform_has_fast_fp16:
        # Check support for OBEY_PRECISION_CONSTRAINTS
        ...
        if precision_type:
            ...
            # Layer types that do not support precision overrides
            ...
            # Numerically sensitive layer types requiring FP32
            ...
            # Model-specific layer name patterns requiring FP32
            # Force FP32 for band_split and final_norm
            fp32_layer_keywords = [
            ...
            for i in range(network.num_layers):
                ...
                # Skip layers that do not support precision overrides
                if layer_type in skip_layer_types:
                    skip_count += 1
                    continue

                # Check for numerically sensitive operations by layer type or name
                is_sensitive = layer_type in fp32_layer_types or any(
                    keyword in layer_name for keyword in fp32_layer_keywords
                )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--bf16", action="store_true", help="Enable BF16 (recommended, better stability)"
    )
    ...
    # Choose the filename according to precision
