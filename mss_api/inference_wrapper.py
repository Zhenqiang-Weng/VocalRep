...
class AudioSeparator:
    def __init__(
        ...
        **kwargs,
        ...
        if self.args.force_cpu:
            ...
            # In multiprocessing mode, device_ids must contain only one ID
            ...
            # Print initialization details only outside distributed workers
        ...
        if self.args.start_check_point:
            ...
            checkpoint = torch.load(
                self.args.start_check_point, map_location="cpu", weights_only=False
            )
            load_start_checkpoint(self.args, self.model, checkpoint, type_="inference")

        if (
            isinstance(self.args.device_ids, list)
            and len(self.args.device_ids) > 1
            and not self.args.force_cpu
        ):
    ...
    def _load_audio(self, path: str):
        ...
        if is_compressed:
            try:
                return self._load_audio_ffmpeg(path, sr)
            except:
                pass
        else:
            try:
                ...
                if orig_sr != sr:
                    audio = _resample_np(audio, orig_sr, sr)
                return audio, sr
            except:
                try:
                    return self._load_audio_ffmpeg(path, sr)
    ...
    def separate_audio(self, audio_path: str):
        ...
        norm_params = None
        if getattr(self.config, "inference", None) is not None and self.config.inference.get(
            "normalize", False
        ):
        ...
        with torch.inference_mode():
            with self._maybe_autocast_ctx():
                # Print model architecture and parameter precision
                # self._print_model_info_once()

                # Check autocast using tensor precision during the forward pass
                ...
                waveforms = demix_with_spk_2(
                    ...
                    apply_gate=self.args.apply_gate,
                    gate_interval=self.args.gate_interval,
                    gate_threshold=self.args.gate_threshold,
                )

        if (
            getattr(self.config, "inference", None) is not None
            and self.config.inference.get("normalize", False)
    ...
    def _print_model_info_once(self):
        """Print model architecture and parameter precision once"""
    ...
    def _check_autocast_active(self):
        """Check whether autocast is active during the forward pass"""
        if hasattr(self, "_autocast_checked"):
            return
        self._autocast_checked = True

        try:
            # Create a small tensor to test matrix multiplication
            ...
            # Run the operation in an autocast context
    ...
    def inference_file(self, audio_path: str, output_root_dir: str):
        ...
        # Measure elapsed time and log the result
    ...
    def inference_folder(self, input_folder: str, output_root_dir: str):
        ...
        for path in tqdm(files, desc="Batch Inference"):
            try:
                self.inference_file(path, output_root_dir)
            except Exception as e:
                print(f"[Batch] Error {path}: {e}")
...
def _worker_process(gpu_id, file_list, args_dict, output_dir):
    """
    Worker: initialize the model and process assigned files
    """
    # Each process sees only its assigned GPU
    os.environ["CUDA_VISIBLE_DEVICES"] = str(gpu_id)
    device_id = 0  # The local GPU ID becomes 0

    # Rebuild AudioSeparator in the child process
    # args_dict contains configuration passed from the parent process
    try:
        separator = AudioSeparator(
            config_path=args_dict["config_path"],
            model_path=args_dict["start_check_point"],
            model_type=args_dict["model_type"],
            device_ids=[device_id],  # Pass only one ID
            force_cpu=False,
            ...
            verbose=False,  # Reduce worker logging
        )

        # Use worker logs or one overall progress bar instead of multiple positioned tqdm bars
        # Use simple log messages in worker processes
...
def run_distributed_inference(txt_path, output_dir, device_ids, args_namespace):
    """
    Split tasks and distribute them across GPUs
    """
    if not os.path.exists(txt_path):
    ...
    # 2. Split the list evenly
    chunk_size = math.ceil(total_files / num_gpus)
    chunks = [files[i : i + chunk_size] for i in range(0, total_files, chunk_size)]

    # 3. Convert Namespace arguments to a dictionary for worker processes
    args_dict = vars(args_namespace)

    # 4. Start processes
    processes = []
    mp.set_start_method("spawn", force=True)  # CUDA requires the spawn start method

    for i, gpu_id in enumerate(device_ids):
        if i >= len(chunks):
            break  # Handle more GPUs than task chunks

        file_chunk = chunks[i]
        p = mp.Process(target=_worker_process, args=(gpu_id, file_chunk, args_dict, output_dir))
        p.start()
        processes.append(p)
...
if __name__ == "__main__":
    ...
    # Accept multiple IDs, such as --device_ids 0 1 2 3
    parser.add_argument("--device_ids", nargs="+", type=int, default=[0])
    parser.add_argument(
        "--spk_model_path", type=str, default="ckpt/speech_campplus_sv_zh_en_16k-common_advanced"
    )
    ...
    if args.input_list and len(args.device_ids) > 1 and not args.force_cpu:
        ...
        # Single-process inference with one initialization
        separator = AudioSeparator(
        ...
        if args.input_list:
            separator.inference_from_list(args.input_list, args.store_dir)
        elif args.input_folder:
            separator.inference_folder(args.input_folder, args.store_dir)
        elif args.input_file:
            separator.inference_file(args.input_file, args.store_dir)
        else:
            print("Please provide --input_folder, --input_file, or --input_list")
