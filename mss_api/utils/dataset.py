...
def prepare_data(config: Dict, args: argparse.Namespace, batch_size: int) -> DataLoader:
    ...
    if dist.is_initialized():
        ...
        trainset = MSSDataset(
            ...
            * world_size,  # maintain "num_steps" semantics across the whole world
...
class MSSDataset(torch.utils.data.Dataset):
            ...
            if type(self.data_path) == list:
                for tp in self.data_path:
                    ...
                    if len(tracks_for_folder) == 0 and should_print:
                        print(
                            "Warning: no tracks found in folder '{}'. Please check it!".format(tp)
                        )
    ...
    def load_source(self, metadata, instr):
        ...
        source = ensure_two_channels(source)
        # Return audio data and track_path
    ...
    def load_source_from_path(self, track_path, instr, track_length=None):
        """Load the requested instrument track from a path

        Args:
            track_path: Track directory (dataset_type 1,4) or audio file (dataset_type 2,3)
            instr: Instrument name
            track_length: Optional track length, detected automatically if omitted

        Returns:
            torch.Tensor: Audio data with shape (2, chunk_size)
        """
        should_print = not dist.is_initialized() or dist.get_rank() == 0

        if self.dataset_type in [1, 4]:
            # track_path is a directory
            for extension in self.file_types:
                path_to_audio_file = track_path + "/{}.{}".format(instr, extension)
                if os.path.isfile(path_to_audio_file):
                    try:
                        ...
                        if should_print:
                            print(
                                "Error loading from path: {} Path: {}".format(e, path_to_audio_file)
                            )
                        source = np.zeros((2, self.chunk_size), dtype=np.float32)
                    break
        else:
            # track_path is an audio file
            # Construct the correct path from instr
            # Assume the path format is: base_path/instr/file.wav
            base_path = os.path.dirname(track_path)
            file_name = os.path.basename(track_path)
            # Replace the instrument name in the filename
            # Assume the filename contains the instrument name; adapt for other naming schemes
            path_to_audio_file = os.path.join(os.path.dirname(base_path), instr, file_name)

            if not os.path.isfile(path_to_audio_file):
                # Try alternative path formats if the file is missing
                path_to_audio_file = track_path.replace(
                    os.path.basename(os.path.dirname(track_path)), instr
                )
        ...
        # Skip augmentation when loading additional tracks
        source = ensure_two_channels(source)
        return torch.tensor(source, dtype=torch.float32)

    def load_random_mix(self):
        res = []
        track_paths = []  # Store track_path for each stem
        ...
        # Load additional mix tracks relative to the first track_path
        extra_sources = []
        if hasattr(self.config.training, "mix_instruments") and len(track_paths) > 0:
            base_track_path = track_paths[0]  # Use the first track path as the base
            for instr in self.config.training.mix_instruments:
                if instr not in self.instruments:
                    try:
                        ...
                        # Skip the track if loading fails
                        should_print = not dist.is_initialized() or dist.get_rank() == 0
                        if should_print:
                    ...
                    for extension in self.file_types:
                        ...
                        if os.path.isfile(path_to_audio_file):
                            try:
                                ...
                                if should_print:
                                    print(
                                        "Error loading extra source: {} Path: {}".format(
    ...
    def __getitem__(self, index):
        ...
        res = all_sources[: len(res)]  # Select the stems to separate
        mix = all_sources.sum(0)  # Generate the mixture from all tracks
