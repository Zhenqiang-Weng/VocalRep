...
AVAILABLE_MODELS = [
    "bs_roformer_exportable",
    "spk_bs_roformer_exportable",
    "speaker_mel_band_roformer_exportable",
]
        ...
        while start < total_samples:
            ...
            if actual_len < chunk_size:
                chunk = torch.nn.functional.pad(
                    chunk, (0, chunk_size - actual_len), mode="constant", value=0
                )
...
def train_model(args):
    ...
    parser = build_training_parser(
        "Train a speaker-guided separator with a discriminator.",
    ...
    parser.add_argument(
        ...
        default="",
        help="Optional discriminator checkpoint.",
    )
    ...
    scheduler = ReduceLROnPlateau(
        optimizer, "max", patience=config.training.patience, factor=config.training.reduce_factor
    )
    ...
    stft_cfg = STFTConfig(
        ...
        multi_window_sizes=tuple(
            getattr(
                config.model, "multi_stft_resolutions_window_sizes", (4096, 2048, 1024, 512, 256)
            )
        ),
    ...
    if args.use_discriminator:
        ...
        if gan_models is None:
            accelerator.print(
                "Warning: gan_model not found in config, using default 'wave' for all stems"
            )
            gan_models = ["wave"] * num_stems
        elif len(gan_models) != num_stems:
            raise ValueError(
                f"gan_model length ({len(gan_models)}) must match number of instruments ({num_stems})"
            )
    ...
    for epoch in range(config.training.num_epochs):
        ...
        for i, (batch, mixes, spk_emb) in enumerate(pbar):
            ...
            if args.use_mix_consistent_loss:
                ...
                total_mix_consistent_loss_weight = getattr(
                    config.training, "total_mix_consistent_loss_weight", 0.5
                )
            ...
            if args.use_discriminator:
                ...
                for stem_idx in range(num_stems):
                    ...
                    g_losses = disc.get_generator_losses(
                        fake_input, real_input, None, current_step=current_step
                    )
